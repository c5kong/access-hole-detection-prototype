function [ACCEPTEDNEW,PixelsBlobNew,PixelsBoundingNew]=REMOVEREGIONS(LLFITTING,ACCEPTED,PixelsBlob,PixelsBounding,im,CMAP);
...
if min(size(COMMONMATRIX))>0
    ...
    while flag
        ...
        if (max(size(M))>1)&&(A(i)~=0)
            ...
            if min(size(REGIONS))>1
                for jj=1:max(size(REGIONS))
                    ...
                    vertices=PixelsBounding{REGIONS(jj,1)};
                    countt=0;
                    if max(size(vertices))>1
                        ...
                        DETECTED2(PixelsBounding{REGIONS(jj,1)})=1;
        ...
        if i==max(size(A(:)))
            i=1;
            % [COMMONMATRIX,LABELCOMMON]=FINDCOMMONLABEL(LLFITTING,ACCEPTEDNEW,im,CMAP);
    ...
    if min(size(MROW))>0
        for i=1:max(size(MROW(:)))
            WHOLEREGIONS=zeros(LENGTH,WIDTH);
            for nasnas=1:max(size(PixelsBoundingNew))
                vertices=PixelsBoundingNew{nasnas};
                if min(size(vertices))>0
                    WHOLEREGIONS(vertices)=1;
                end
            end
            count=0;
            MM=find(COMMONMATRIX(MROW(i),:)>0);
                ...
                for jj=1:max(size(REGIONS))
                    ...
                    if max(size(vertices))>1
                        ...
                        if jj==1;
                            ...
                            MERGED=in222>0;
                            MERGEDLABEL=bwlabel(MERGED);
                            hhh=regionprops(MERGEDLABEL,'all');
                            vertices=hhh(1).ConvexHull;
                            in=inpolygon(XX(:),YY(:),vertices(:,1),vertices(:,2));
                            DETECTED=reshape(in,LENGTH,WIDTH);
                            XMIN=hhh(1).BoundingBox(1);
                            XMAX=hhh(1).BoundingBox(1)+hhh(1).BoundingBox(3);
            ...
            if min(size(REGIONS))>1
                if ~any(any((bitand(DETECTED2,(WHOLEREGIONS-MERGED)))))
                    ACCEPTEDNEW{1,REGIONS(2)}=[];
                    ACCEPTEDNEW{1,REGIONS(1)}=hhh(1).ConvexHull;
                    PixelsBlobNew{REGIONS(2)}=[];
                    PixelsBlobNew{REGIONS(1)}=find(DETECTED==1);
                    PixelsBoundingNew{REGIONS(2)}=[];
                    PixelsBoundingNew{REGIONS(1)}=find(DETECTED2==1);
                else
                    PNORMALIZED=PMAP./AREA;
                    [MJ,NJ]=min(PNORMALIZED);
                    ACCEPTEDNEW{1,REGIONS(NJ,1)}=[];
